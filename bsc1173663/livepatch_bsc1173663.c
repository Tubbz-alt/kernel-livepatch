...
int klpp_intel_engine_cmd_parser(struct i915_gem_context *cxt,
			    struct intel_engine_cs *engine,
...
static bool klpr_check_cmd(const struct intel_engine_cs *engine,
		      const struct drm_i915_cmd_descriptor *desc,
...
static int klpr_check_bbstart(const struct i915_gem_context *ctx,
...
static void klpp_init_whitelist(struct i915_gem_context *ctx, u32 batch_len)
{
			    ...
			    u64 shadow_batch_start)
{
	...
	klpp_init_whitelist(ctx, batch_len);
	...
	do {
		...
		if (!klpr_check_cmd(engine, desc, cmd, length)) {
			ret = -EACCES;
			goto err;
		}

		if (desc->cmd.value == MI_BATCH_BUFFER_START) {
			ret = klpr_check_bbstart(ctx, cmd, offset, length,
