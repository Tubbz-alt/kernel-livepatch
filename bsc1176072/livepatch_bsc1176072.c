...
static void (*klpe___packet_set_status)(struct packet_sock *po, void *frame, int status);

static int (*klpe___packet_get_status)(struct packet_sock *po, void *frame);

static __u32 (*klpe_tpacket_get_timestamp)(struct sk_buff *skb, struct timespec *ts,
				   unsigned int flags);

static void *klpr_packet_lookup_frame(struct packet_sock *po,
		...
		int status)
{
	...
	if (status != (*klpe___packet_get_status)(po, h.raw))
		return NULL;
					    ...
					    int status, unsigned int len)
{
	...
	case TPACKET_V1:
	case TPACKET_V2:
		curr = klpr_packet_lookup_frame(po, &po->rx_ring,
					po->rx_ring.head, status);
		return curr;
	case TPACKET_V3:
		return klpr___packet_lookup_frame_in_block(po, skb, status, len);
	default:
		       ...
		       struct packet_type *pt, struct net_device *orig_dev)
{
	...
	if (po->tp_version <= TPACKET_V2) {
		(*klpe___packet_set_status)(po, h.raw, status);
		sk->sk_data_ready(sk);
	} else {
...
static struct klp_kallsyms_reloc klp_funcs[] = {
	...
	{ "__packet_set_status", (void *)&klpe___packet_set_status,
	  "af_packet" },
	{ "__packet_get_status", (void *)&klpe___packet_get_status,
	  "af_packet" },
