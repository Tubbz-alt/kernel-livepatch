...
static inline ssize_t
klpr_xdr_stream_decode_uint32_array(struct xdr_stream *xdr,
		__u32 *array, size_t array_size)
{
	...
	if (unlikely(klpr_xdr_stream_decode_u32(xdr, &len) < 0))
		return -EBADMSG;
	p = (*klpe_xdr_inline_decode)(xdr, len * sizeof(*p));
	if (unlikely(!p))
		return -EBADMSG;
	if (array == NULL)
		return len;
...
static int klpr_decode_opaque_inline(struct xdr_stream *xdr, unsigned int *len, char **string)
{
	ssize_t ret = klpr_xdr_stream_decode_opaque_inline(xdr, (void **)string,
			NFS4_OPAQUE_LIMIT);
	if (unlikely(ret < 0))
		return -EIO;
	*len = ret;
...
static ssize_t
klpr_decode_bitmap4(struct xdr_stream *xdr, uint32_t *bitmap, size_t sz)
{
	ssize_t ret;

	ret = klpr_xdr_stream_decode_uint32_array(xdr, bitmap, sz);
	if (likely(ret >= 0))
		return ret;
	if (ret != -EMSGSIZE)
		return -EIO;
...
static int klpr_decode_attr_bitmap(struct xdr_stream *xdr, uint32_t *bitmap)
{
	ssize_t ret;
	ret = klpr_decode_bitmap4(xdr, bitmap, 3);
	return ret < 0 ? ret : 0;
}
		...
		for (loc->nservers = 0; loc->nservers < m; loc->nservers++) {
			...
			if (loc->nservers == NFS4_FS_LOCATION_MAXSERVERS) {
				...
				for (i = loc->nservers; i < m; i++) {
					unsigned int len;
					char *data;
					status = klpr_decode_opaque_inline(xdr, &len, &data);
					if (unlikely(status != 0))
						goto out_eio;
				}
				break;
			...
			status = klpr_decode_opaque_inline(xdr, &server->len, &server->data);
			if (unlikely(status != 0))
				goto out_eio;
		...
		struct nfs4_string *owner_name)
{
	...
	*uid = make_kuid(&init_user_ns, -2);
	...
	if (owner_name != NULL) {
		len = klpr_decode_nfs4_string(xdr, owner_name, GFP_NOIO);
		if (len <= 0)
			goto out;
		...
		struct nfs4_string *group_name)
{
	...
	*gid = make_kgid(&init_user_ns, -2);
	...
	if (group_name != NULL) {
		len = klpr_decode_nfs4_string(xdr, group_name, GFP_NOIO);
		if (len <= 0)
			goto out;
...
static int klpr_decode_attr_time(struct xdr_stream *xdr, struct timespec *time)
{
	__be32 *p;

	p = (*klpe_xdr_inline_decode)(xdr, nfstime4_maxsz << 2);
	if (unlikely(!p))
		return -EIO;
...
static int klpr_decode_attr_time_access(struct xdr_stream *xdr, uint32_t *bitmap, struct timespec *time)
{
	...
	if (likely(bitmap[1] & FATTR4_WORD1_TIME_ACCESS)) {
		status = klpr_decode_attr_time(xdr, time);
		if (status == 0)
					...
					struct nfs4_label *label)
{
	...
	if (label && label->label)
		dprintk("%s: label=%s, len=%d, PI=%d, LFS=%d\n",
			__func__, (char *)label->label, label->len, label->pi, label->lfs);
	return status;
}
...
static int klpr_decode_threshold_hint(struct xdr_stream *xdr,
				  uint32_t *bitmap,
				  uint64_t *res,
				  uint32_t hint_bit)
{
					...
					struct nfs4_threshold *res)
{
	...
	/* thi_hintset bitmap */
	status = klpr_decode_attr_bitmap(xdr, bitmap);
	if (status < 0)
		goto xdr_error;
	...
	if (status < 0)
		goto xdr_error;
	/* thi_hintlist */
	status = klpr_decode_threshold_hint(xdr, bitmap, &res->rd_sz, THRESHOLD_RD);
	if (status < 0)
		goto xdr_error;
	status = klpr_decode_threshold_hint(xdr, bitmap, &res->wr_sz, THRESHOLD_WR);
	if (status < 0)
		goto xdr_error;
	status = klpr_decode_threshold_hint(xdr, bitmap, &res->rd_io_sz,
				       THRESHOLD_RD_IO);
	if (status < 0)
		goto xdr_error;
	status = klpr_decode_threshold_hint(xdr, bitmap, &res->wr_io_sz,
				       THRESHOLD_WR_IO);
	if (status < 0)
		goto xdr_error;
