...
/* klp-ccp: from net/bluetooth/l2cap_core.c */
static u16 (*klpe_le_max_credits);
...
static void klpr_l2cap_chan_le_send_credits(struct l2cap_chan *chan)
{
	 ...
	 */
	if (chan->rx_credits >= ((*klpe_le_max_credits) + 1) / 2)
		return;

	return_credits = (*klpe_le_max_credits) - chan->rx_credits;
	...
	chan->ident = (*klpe_l2cap_get_ident)(conn);

	(*klpe_l2cap_send_cmd)(conn, chan->ident, L2CAP_LE_CREDITS, sizeof(pkt), &pkt);
}
...
static int klpr_l2cap_le_data_rcv(struct l2cap_chan *chan, struct sk_buff *skb)
{
	...
	klpr_l2cap_chan_le_send_credits(chan);
...
static struct klp_kallsyms_reloc klp_funcs[] = {
	...
	{ "le_max_credits", (void *)&klpe_le_max_credits, "bluetooth" },
