...
static int (*klpe_l2cap_le_recv)(struct l2cap_chan *chan, struct sk_buff *skb);
...
static int klpr_l2cap_le_data_rcv(struct l2cap_chan *chan, struct sk_buff *skb)
{
	...
	if (!chan->sdu) {
		...
		if (skb->len == sdu_len)
			return (*klpe_l2cap_le_recv)(chan, skb);
	...
	if (chan->sdu->len == chan->sdu_len) {
		err = (*klpe_l2cap_le_recv)(chan, chan->sdu);
		if (!err) {
			chan->sdu = NULL;
			chan->sdu_last_frag = NULL;
			chan->sdu_len = 0;
...
static struct klp_kallsyms_reloc klp_funcs[] = {
	...
	{ "l2cap_le_recv", (void *)&klpe_l2cap_le_recv, "bluetooth" },
};
