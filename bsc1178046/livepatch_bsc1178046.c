...
/* klp-ccp: from fs/btrfs/extent-tree.c */
...
static u64 (*klpe_get_alloc_profile)(struct btrfs_fs_info *fs_info, u64 orig_flags);

/* New. */
u64 klpp_btrfs_metadata_alloc_profile(struct btrfs_fs_info *fs_info)
{
	return (*klpe_get_alloc_profile)(fs_info, BTRFS_BLOCK_GROUP_METADATA);
}

/* New. */
u64 klpp_btrfs_system_alloc_profile(struct btrfs_fs_info *fs_info)
{
	return (*klpe_get_alloc_profile)(fs_info, BTRFS_BLOCK_GROUP_SYSTEM);
}
			  ...
			  bool system_chunk)
{
	 ...
	 *  -1 line, +4 lines
	 *
	 * Note: system_chunk is either set to (root == fs_info->chunk_root)
	 * in reserve_metadata_bytes() or to constant false when
	 * can_overcommit() had previously been invoked with a known
	 * root != ->chunk_root. Thus, the resulting profile will be
	 * the same as before and the only change is that accesses to
	 * root's ->fs_info is avoided.
	 * (Actually, the path through reserve_metadata_bytes() is left
	 *  out from this livepatch as the root should better be valid
	 *  there).
	 */
	if (system_chunk)
		profile = klpp_btrfs_system_alloc_profile(fs_info);
	else
		profile = klpp_btrfs_metadata_alloc_profile(fs_info);
	used = (*klpe_btrfs_space_info_used)(space_info, false);
...
void klpp_btrfs_async_reclaim_metadata_space(struct work_struct *work)
{
	...
	do {
		...
		ret = (*klpe_flush_space)(fs_info, space_info, to_reclaim, to_reclaim,
				  flush_state);
		spin_lock(&space_info->lock);
...
static struct klp_kallsyms_reloc klp_funcs[] = {
	{ "get_alloc_profile", (void *)&klpe_get_alloc_profile,
	  "btrfs" },
