...
/* klp-ccp: from drivers/tty/vt/vt_ioctl.c */
static inline bool klpr_vt_in_use(unsigned int i)
{
...
static inline bool klpr_vt_busy(int i)
{
	if (klpr_vt_in_use(i))
		return true;
	if (i == fg_console)
		return true;
	if ((*klpe_vc_is_sel)(vc_cons[i].d))
		return true;
...
/* klp-ccp: from include/linux/syscalls.h */
static long (*klpe_sys_ioperm)(unsigned long from, unsigned long num, int on);
	     ...
	     unsigned int cmd, unsigned long arg)
{
	...
	case KDDELIO:
		...
		ret = (*klpe_sys_ioperm)(arg, 1, (cmd == KDADDIO)) ? -ENXIO : 0;
		break;

	case KDENABIO:
	case KDDISABIO:
		ret = (*klpe_sys_ioperm)(GPFIRST, GPNUM,
	     ...
	     unsigned int cmd, unsigned long arg)
{
	...
	case GIO_FONTX:
		 ...
		 *  -1 line, +1 line
		 */
		ret = klpp_compat_fontx_ioctl(vc, cmd, up, perm, &op);
		break;

	case KDFONTOP:
		ret = klpr_compat_kdfontop_ioctl(up, perm, &op, vc);
		break;

	case PIO_UNIMAP:
	case GIO_UNIMAP:
		ret = klpr_compat_unimap_ioctl(cmd, up, perm, vc);
		break;
...
#ifdef CONFIG_X86
	{ "sys_ioperm", (void *)&klpe_sys_ioperm },
#endif
